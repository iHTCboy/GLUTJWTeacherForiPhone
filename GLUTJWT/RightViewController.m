...
- (void)viewDidLoad
{
    ...
    headImageView.frame = CGRectMake(self.view.center.x * 1.12, y, 100, 100);
    ...
    //学号、名字
    UILabel * nameLabel = [[UILabel alloc]initWithFrame:CGRectMake(self.view.center.x, self.view.center.y * 0.68, 150, 40)];
    ...
    UILabel * numberLabel = [[UILabel alloc]initWithFrame:CGRectMake(self.view.center.x , self.view.center.y * 0.8, 150, 40)];
    ...
    // numberLabel.text = @"20140901";
    
    //职务
    UILabel * worksLabel = [[UILabel alloc]initWithFrame:CGRectMake(self.view.center.x, self.view.center.y * 0.9, 150, 40)];
    worksLabel.backgroundColor = [UIColor clearColor];
    worksLabel.font = [UIFont boldSystemFontOfSize:18];
    ...
    //worksLabel.text = @"教授";
     ...
     //科室
    UILabel * classLabel = [[UILabel alloc]initWithFrame:CGRectMake(self.view.center.x, self.view.center.y, 150, 40)];
    ...
    //classLabel.text = @"网络工程教研室";
    
    UILabel * collegeLabel = [[UILabel alloc]initWithFrame:CGRectMake(self.view.center.x * 0.9, self.view.center.y * 1.1, 170, 40)];
    ...
    //collegeLabel.text = @"计算机科学与工程学院";
    
    
    //取消按钮
    UIButton * cancel =[[UIButton alloc]initWithFrame:CGRectMake(self.view.center.x * 0.95, self.view.center.y * 1.4, 150, 30)];
    ...
    [infoBtn setTitle:@" 关于" forState:UIControlStateNormal];
    ...
    if ([defaults valueForKeyPath:@"numberLabel"])
    {
        ...
        self.worksLabel.text = [defaults valueForKeyPath:@"worksLabel"];
...
//    NSString * nameTitle = [urlString substringWithRange:nameRange];
    ...
    //获取职务
    self.worksLabel.text = (NSString *) [[[[strUrl2 componentsSeparatedByString:@"职务</th><td>"] objectAtIndex:1] componentsSeparatedByString:@"&"] objectAtIndex:0];
    
    [defaults setValue:self.worksLabel.text forKeyPath:@"worksLabel"];
    [defaults synchronize];
...
//    NSString * classTitle = [urlString substringWithRange:classRange];
     self.classLabel.text = (NSString *) [[[[strUrl2 componentsSeparatedByString:@"教研组</th><td>"] objectAtIndex:1] componentsSeparatedByString:@"&"] objectAtIndex:0];
...
//    NSString * collegeTitle = [urlString substringWithRange:collegeRange];
    ...
    NSString * imageStr = (NSString *) [[[[strUrl2 componentsSeparatedByString:@"imgsrc=\""] objectAtIndex:1] componentsSeparatedByString:@"\""] objectAtIndex:0];
    
    [defaults setValue:imageStr forKeyPath:@"imageStr"];
    [defaults synchronize];
...
-(void)reloadhead
{
    ...
    NSString * imageStr = [defaults valueForKey:@"imageStr"];
    
    NSString * imageURL = [NSString stringWithFormat:@"http://202.193.80.58:81%@",imageStr];

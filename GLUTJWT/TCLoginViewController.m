...
- (void)viewDidLoad
{
    ...
    logo.image = [UIImage imageNamed:@"loginRed.png"];
    ...
    //记住密码功能
    UILabel * holdPassword = [[UILabel alloc ]initWithFrame:CGRectMake(self.view.frame.size.width * 0.58, self.view.center.y * 1.1/2 + 165, 70, 30)];
    ...
    UISwitch * switchPassword = [[UISwitch alloc]initWithFrame:CGRectMake(self.view.frame.size.width * 0.58 + 70, self.view.center.y * 1.1/2 + 165, 51, 31)];
    ...
    //    //离线模式
    UILabel * logoutLabel = [[UILabel alloc ]initWithFrame:CGRectMake(self.view.frame.size.width * 0.06, self.view.center.y * 1.1/2 + 165, 70, 30)];
    ...
    UISwitch * switchLogout = [[UISwitch alloc]initWithFrame:CGRectMake(self.view.frame.size.width * 0.06 +70, self.view.center.y * 1.1/2 + 165, 80, 31)];
    ...
    //隐私政策
    UIButton * yBut = [[UIButton alloc]initWithFrame:CGRectMake(self.view.frame.size.width * 0.15, self.view.center.y * 1.1/2 + 199, 20, 20)];
    [yBut setBackgroundImage:[UIImage imageNamed:@"tbutNO.png"] forState:UIControlStateNormal];
    [yBut setBackgroundImage:[UIImage imageNamed:@"tbut.png"] forState:UIControlStateSelected];
    ...
    UILabel * sLabel = [[UILabel alloc ]initWithFrame:CGRectMake(self.view.frame.size.width * 0.15 +25, self.view.center.y * 1.1/2 + 195, 105, 30)];
    sLabel.text = @"我已经阅读并同意";
    sLabel.font = [UIFont systemFontOfSize:13];
    sLabel.textColor = [UIColor whiteColor];
    [self.view addSubview:sLabel];
    
    UIButton * sBut = [[UIButton alloc]initWithFrame:CGRectMake(self.view.frame.size.width * 0.15 + 132, self.view.center.y * 1.1/2 + 195, 120, 30)];
    [sBut setTitle:@"使用条款和隐私政策" forState:UIControlStateNormal];
    [sBut setTitleColor:TCCoror(255, 0, 0) forState:UIControlStateNormal];
...
-(void)switchLogouted
{
    ...
    if (self.switchLogout.isOn)
    {
        ...
        //判断是否提示离线模式的功能
        if (![defaults boolForKey:@"neverShow"])
        {
...
-(void)loadYZM
{
    ...
    NSURL *url = [NSURL URLWithString:@"http://202.193.80.58:81/academic/getCaptcha.do"];
    ...
    UIImage * newImage = [UIImage  imageWithData:imageDate];
     ...
     self.imageYZM.image = [self imageFromImage:newImage inRect:CGRectMake(0, 0,70, 17)];
